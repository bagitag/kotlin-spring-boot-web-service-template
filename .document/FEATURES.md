...
> [!NOTE]
> This feature is managed by the `management.tracing.export.enabled` property in the `application.properties` file. If set to `true`, a higher level tracing solution is used and request ID generation is not required. See [OBSERVABILITY.md](OBSERVABILITY.md) for more details.
